...
BOOL shouldUseCache(NSString *file, double seconds) {
	...
	if(([age timeIntervalSinceNow] * -1) > seconds && [((AppDelegate *)[UIApplication sharedApplication].delegate) hasNetworkConnection]) {
		return NO;
	} else
		return YES;
}
...
- (NSArray *)doGet:(NSString *)url maxCacheAge:(double)seconds XPath:(NSString *)XPath {
	...
	} else if([((AppDelegate *)[UIApplication sharedApplication].delegate) hasNetworkConnection] && !cacheOnly) {
		[UIApplication sharedApplication].networkActivityIndicatorVisible = YES;
		NSMutableURLRequest *theRequest = [NSMutableURLRequest requestWithURL:[NSURL URLWithString:url] cachePolicy:NSURLRequestReloadIgnoringCacheData timeoutInterval:[((AppDelegate *)[UIApplication sharedApplication].delegate) hasWiFiConnection]?40:60];
...
- (NSArray *)_doMethod:(NSString *)method maxCacheAge:(double)seconds XPath:(NSString *)XPath withParams:(NSArray *)params authenticated:(BOOL)authenticated {
	...
	if((seconds && shouldUseCache(CACHE_FILE([signature md5sum]),seconds)) || cacheOnly) {
		...
		[UIApplication sharedApplication].networkActivityIndicatorVisible = YES;
		NSMutableURLRequest *theRequest = [NSMutableURLRequest requestWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%s", API_URL]] cachePolicy:NSURLRequestReloadIgnoringCacheData timeoutInterval:[((AppDelegate *)[UIApplication sharedApplication].delegate) hasWiFiConnection]?40:60];
		[theRequest setValue:kUserAgent forHTTPHeaderField:@"User-Agent"];
...
- (NSDictionary *)getPlaylist {
	...
	if([((AppDelegate *)[UIApplication sharedApplication].delegate) hasWiFiConnection]) {
		network = @"wifi";
		bitrate = @"128";
		speed = @"8";
	} else {
