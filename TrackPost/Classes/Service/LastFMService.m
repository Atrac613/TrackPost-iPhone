...
- (NSArray *)_doMethod:(NSString *)method maxCacheAge:(double)seconds XPath:(NSString *)XPath withParams:(NSArray *)params authenticated:(BOOL)authenticated {
	...
	} else if([((TrackPostAppDelegate *)[UIApplication sharedApplication].delegate) hasNetworkConnection] && !cacheOnly) {
		[UIApplication sharedApplication].networkActivityIndicatorVisible = YES;
		NSMutableURLRequest *theRequest = [NSMutableURLRequest requestWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%s", API_URL]] cachePolicy:NSURLRequestReloadIgnoringCacheData timeoutInterval:[((TrackPostAppDelegate *)[UIApplication sharedApplication].delegate) hasWiFiConnection]?40:60];
